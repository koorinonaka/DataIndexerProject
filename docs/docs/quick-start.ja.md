...
Schemaと Repository の作成から行のオーサリング、ランタイムでのQueryまでを一通り説明します。
...
<div class="qs-prereq">
  ...
  <div class="qs-prereq-grid">
    ...
    <div class="qs-prereq-row">
      <span class="qs-prereq-check"><svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.5"><path d="m3 8 3 3 7-7"/></svg></span>
      <span><strong>Plugin有効化</strong><span class="qs-prereq-meta">→ <a href="https://dev.epicgames.com/documentation/unreal-engine/working-with-plugins-in-unreal-engine">（公式ドキュメント）</a></span></span>
    </div>
    <div class="qs-prereq-row">
      <span class="qs-prereq-check"><svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.5"><path d="m3 8 3 3 7-7"/></svg></span>
...
<div class="qs-progress" role="navigation" aria-label="進捗">
  <div class="qs-progress-rail">
    ...
    <div class="qs-progress-step" data-step="1">
      ...
      <div class="qs-progress-label">Schema作成</div>
    </div>
...
## 構造体とSchemaを作成 { #step-1 .step }

=== "C++"

    **行構造体を定義**{ .step-label }

    行データを格納する構造体と、型安全なQuery用のエイリアスを宣言します：
...
<div class="qs-complete">
  ...
  <ul class="qs-complete-list">
    <li>
      ...
      <span><strong>行データの型定義</strong> — 行データを表す構造体が定義されている</span>
    </li>
    <li>
      <span class="qs-complete-check"><svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.5"><path d="m3 8 3 3 7-7"/></svg></span>
      <span><strong>Schema設定済み</strong> — Schemaが行構造体に紐付けられている</span>
    </li>
    <li>
      <span class="qs-complete-check"><svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.5"><path d="m3 8 3 3 7-7"/></svg></span>
...
1. 右クリック → **Miscellaneous → DataIndexer** Assetを選択
...
![Insert Keyで行を追加](assets/images/data-view-add-row.png)
...
<div class="qs-complete">
  ...
  <ul class="qs-complete-list">
    <li>
      <span class="qs-complete-check"><svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.5"><path d="m3 8 3 3 7-7"/></svg></span>
      <span><strong>Repository Asset</strong> — Schemaにバインドされた状態で作成されている</span>
    </li>
    <li>
      <span class="qs-complete-check"><svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.5"><path d="m3 8 3 3 7-7"/></svg></span>
...
=== "Blueprint"
    ...
    **Get Row** Nodeをグラフにスポーンします：
    ...
    ![Get Row Nodeの例](assets/images/get-row-node.png)

    **全行を反復する**{ .step-label }

    **Get All Primary Keys** に Repository を渡して全プライマリKeyを取得し、**For Each Loop** で各Keyに **Get Row** を呼び出します。

    ![全行を反復するNodeグラフ](assets/images/iterate-all-rows.png){ .qs-zoomable }

    IndexによるFilterリングは [Blueprint API → 関数ライブラリ](api-reference/function-library.md) を参照してください。
<div class="qs-complete">
  <p class="qs-complete-title">
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="9" r="8"/><path d="m5 9 3 3 5-6"/></svg>
    ここまでで達成したこと
  ...
  <ul class="qs-complete-list">
    <li>
      <span class="qs-complete-check"><svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.5"><path d="m3 8 3 3 7-7"/></svg></span>
      <span><strong>基本Query</strong> — Blueprint または C++ から行を取得できる</span>
    </li>
    <li>
      <span class="qs-complete-check"><svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.5"><path d="m3 8 3 3 7-7"/></svg></span>
...
- :material-puzzle-outline:{ .lg .middle } &nbsp; **[Blueprint API](api-reference/index.md)**
    ...
    Blueprint Node・関数ライブラリ・Driven Collection の完全リファレンス。

- :material-language-cpp:{ .lg .middle } &nbsp; **[C++ API](api-reference/index.md)**

    ---

    型安全なQueryパターンと Native Schema Interface。
...
<div class="qs-lightbox" id="qs-lightbox" onclick="qsCloseLightbox()">
  <button class="qs-lightbox-close" onclick="qsCloseLightbox()" aria-label="閉じる">&#x2715;</button>
  <img class="qs-lightbox-img" src="" alt="拡大表示" onclick="event.stopPropagation()">
</div>
<script>
