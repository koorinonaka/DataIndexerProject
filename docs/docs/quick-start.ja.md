...
Schemaと Repository の作成から行のオーサリング、ランタイムでのクエリまでを一通り説明します。
...
<div class="qs-complete">
  ...
  <ul class="qs-complete-list">
    <li>
      <span class="qs-complete-check"><svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.5"><path d="m3 8 3 3 7-7"/></svg></span>
      <span><strong>Repository アセット</strong> — Schemaにバインドされた状態で作成されている</span>
    </li>
    <li>
      <span class="qs-complete-check"><svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.5"><path d="m3 8 3 3 7-7"/></svg></span>
...
=== "Blueprint"
    ...
    **Get All Primary Keys** に Repository を渡して全PrimaryKeyを取得し、**For Each Loop** で各キーに **Get Row** を呼び出します。

    ![全行を反復するノードグラフ](assets/images/iterate-all-rows.png){ .qs-zoomable }

    Indexによるフィルタリングは [Blueprint API → 関数ライブラリ](api-reference/function-library.md) を参照してください。
<div class="qs-complete">
  <p class="qs-complete-title">
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="9" r="8"/><path d="m5 9 3 3 5-6"/></svg>
    ここまでで達成したこと
...
- :material-graph-outline:{ .lg .middle } &nbsp; **[コアコンセプト](concepts/index.md)**
    ...
    Repository / Schema / キー / Indexes の関係を体系的に理解。
