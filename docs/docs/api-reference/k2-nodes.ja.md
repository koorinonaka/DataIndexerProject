...
DataIndexer は標準の `UFUNCTION` では表現できない 3 種類のカスタム Blueprint グラフノードを提供します。これらのノードはSchemaとRepositoryの設定に基づいてコンパイル時に型付きピンを生成します。
...
**目的：** PrimaryKeyがどの行をアドレスしているかによって実行を分岐します。DataIndexer 行用の `Switch on Enum` ノードに相当します。
...
ノードの Details パネルで **Pin キー** にエントリを追加します。各エントリで `FDataIndexerPrimaryKey` の GUID を指定し、ラベル付きの出力実行ピンが作成されます。
