...
`BaseValue` と `Rarity` を変更不可にしたい場合は、UDStruct でそれらに `NotOverridable` を設定します。子が親行を編集する際、それらのフィールドは読み取り専用のままになります。設定手順は [Repositoryメタデータ → NotOverridable](repository-metadata.md#notoverridable) を参照してください。
...
- [Repository — 親 Repository の合成](../concepts/repository.md#parent-repository-composition)
- [Repositoryメタデータ → NotOverridable](repository-metadata.md#notoverridable)
