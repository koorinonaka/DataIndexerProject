...
`UDataIndexerRepository` は `UDataAsset` のサブクラスで、型付きの行コレクションを格納します。DataIndexer ワークフローの主要な成果物です。デザイナーはここで行をオーサリングし、ランタイムコードはここから行をQueryします。

## 親 Repository の合成

Repositoryは 1 つ以上の親Repositoryを参照できます。走査・Query時には階層が再帰的に辿られ、親の行は子を通して見えるようになります。
...
=== "C++"
    ...
    ネイティブ C++ 構造体の場合は `const` またはアクセス制御を使用してください。UDStruct フィールドの場合、カスタマイズレイヤーがこのKeyを管理します。
...
クラスが `UDataIndexerRepository` UPROPERTY を持ち、特定のSchemaのRepositoryのみに制限したい場合は `meta = (Schema = "AssetPath")` を追加します。エディタはパスを解決し、Assetピッカーを対応するRepositoryのみに絞り込みます。
...
=== "Blueprint"
    ...
    Blueprint 変数の **Details** パネルを開きます。`UDataIndexerRepository` 変数を選択すると **Schema** ピッカーが表示されます。SchemaAssetを選択すると、Assetピッカーが一致するRepositoryのみに絞り込まれます。ピッカーをクリアするとすべてのRepositoryが表示されます。
## シリアライズ

バイナリAsset形式はカスタムの `Serialize` オーバーライドで `LocalEntries`・`EntryOwners`・`ReverseLookups` をシリアライズします。`ReverseLookups` は保存時にSchemaの `BuildIndexFunctions` から再構築されます。
