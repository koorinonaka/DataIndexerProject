...
DataIndexer リポジトリは JSON によるエクスポート・インポート・Diff に対応しています。各操作はコンテンツブラウザのコンテキストメニューから実行します。

## エクスポート

1. コンテンツブラウザでリポジトリアセットを右クリック
2. **Export JSON** を選択
3. 保存先のパスを選択する
...
カスタムエディタで開いたリポジトリはツールバーの **Reimport** ボタンから再インポートできます。
