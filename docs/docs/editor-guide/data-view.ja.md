...
| パネル | 位置 | 役割 |
|--------|------|------|
| Asset Details | 左 | Repositoryレベルのプロパティ（Schema Class、Parent Repositories） |
| Data View | 中央 | 行グリッド — 行の追加・削除・編集 |
| Selection Details | 右 | 選択行のフルプロパティエディタ |

## 行の追加

![Insert Keyで新規行を追加する Data View](../assets/images/data-view-add-row.png)
...
!!! warning "削除は恒久的"
    行を削除するとそのプライマリKeyが削除されます。参照されている行を削除しようとすると確認ダイアログが表示されます。参照ビューアボタンがアクティブな行は、削除前に参照先を確認してください。
...
| 項目 | ショートカット | 説明 |
|------|--------------|------|
| **Copy ID** | — | 選択行のプライマリKey（GUID）をクリップボードにコピー |
...
継承行は子Repositoryでは編集できません — ダブルクリックで親Assetに移動します。

**Override**（`Shift+O`）を実行すると、親行の値をコピーした状態でこのRepositoryに行が追加され、独立して編集できるようになります。

## 参照ビューア

各行の左端に参照状態アイコンが表示されます。アイコンをクリックすると UE5 の **Reference Viewer** が開き、その行を参照しているAssetを一覧できます。

![Reference Viewer ボタン](../assets/images/data-view-reference-viewer.gif)

- アイコンが**アクティブ**な行 — いずれかのAssetから参照されています
