...
リポジトリエディタは 3 つのパネルで構成されます。

| パネル | 位置 | 役割 |
|--------|------|------|
| Asset Details | 左 | リポジトリレベルのプロパティ（Schema Class、Parent Repositories） |
...
Data View はスプレッドシートと同様の**セル単位の選択**を採用しています。選択範囲はハイライト枠で表示されます。

| 操作 | 結果 |
|------|------|
| セルをクリック | そのセルを選択 |
| クリック + ドラッグ | 矩形範囲を選択 |
| Shift+クリック | 最後のアンカーから clicked セルまで選択を拡張 |
| Ctrl+クリック | そのセルの選択をトグル（追加 / 解除） |
| Ctrl+ドラッグ | 矩形範囲をトグル |
| ダブルクリック | そのセルのインライン編集モードに入る |
| Escape | インライン編集モードを終了 |

選択可能なセルにホバーするとカーソルが十字形に変わります。

右の **Selection Details** パネルは選択セルを含む行のプロパティを表示します。行単位の操作（Delete・Duplicate・MoveUp 等）を行うには、データセルを右クリックして **Select Row** を選ぶか、行のドラッグハンドルカラムを右クリックしてください。
...
セル（行全体でなく）を選択しているときに**データセル**を右クリックします。
...
**ドラッグハンドルカラム**を右クリックするか、行全体を選択した状態で右クリックします。
...
!!! tip "データ整理に最適"
    **Filters → Show Only Unreferred Rows** と組み合わせることで、未使用の行を一覧して不要なデータを安全に削除できます。
